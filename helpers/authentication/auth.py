...
def register_details(**kwargs):
    ...
    if not re.match(r"(^[a-zA-Z0-9_.]+@[a-zA-Z0-9-]+\.[a-z]+$)", kwargs['email']):
        # Check email validity
        return 'Provide a valid email!'
    if len(kwargs['password']) < 5 or not re.search("[a-z]", kwargs['password']) or not\
    re.search("[0-9]", kwargs['password']) or not re.search("[A-Z]", kwargs['password']) \
    or not re.search("[$#@]", kwargs['password']):
...
def register(**kwargs):
    ...
    user_name = User.query.filter_by(username=kwargs['username']).first()
